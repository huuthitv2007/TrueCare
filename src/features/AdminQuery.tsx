...
export function AdminQuery({query,members,statuses,archive=false,dates=true}:{query:ReturnType<typeof usePagedQuery<any>>;members?:TeamMember[];statuses?:Record<string,string>;archive?:boolean;dates?:boolean}) {
  ...
  return <><div className="toolbar">
    ...
    {archive&&<select aria-label="Lưu trữ" value={query.draft.archive??"visible"} onChange={event=>update("archive",event.target.value)}><option value="visible">Đang hiển thị</option><option value="archived">Đã lưu trữ</option><option value="all">Tất cả</option></select>}
