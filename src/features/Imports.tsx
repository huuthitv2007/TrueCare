...
export function Imports(){
 const {command,notify}=useWorkspace();const [kind,setKind]=useState<ImportKind>('cost');const [file,setFile]=useState<File|null>(null);const [preview,setPreview]=useState<(ImportPreview & {previewId:string})|null>(null);const [busy,setBusy]=useState(false);const [error,setError]=useState('');const [ack,setAck]=useState(false);const [committed,setCommitted]=useState(false);
 ...
 <Card title="Chọn nguồn dữ liệu" subtitle="Nhập lần lượt giá gốc → giá chào → đơn hàng. Tệp gốc được giữ nguyên."><div className="form-grid"><Field label="Loại dữ liệu"><select value={kind} onChange={e=>{setKind(e.target.value as ImportKind);setPreview(null)}}><option value="cost">Bảng giá gốc · Excel</option><option value="quotes">Bảng giá chào hàng · Excel</option><option value="orders">Đơn hàng · TXT</option></select></Field><Field label="Tệp cần nhập" hint="Tối đa 10 MB, 10.000 dòng. Chỉ XLSX hoặc TXT."><input type="file" accept={kind==='orders'?'.txt':'.xlsx'} onChange={e=>{setFile(e.target.files?.[0]||null);setPreview(null)}}/></Field></div><div className="form-actions"><Button variant="primary" busy={busy} onClick={()=>load()} disabled={!file}><Upload size={17}/>Đọc và đối chiếu tệp</Button><Button busy={busy} onClick={()=>load(true)}><FileSpreadsheet size={17}/>Đọc file trong thư mục TrueCare</Button></div></Card>
