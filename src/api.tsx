...
export interface WorkspaceContext {state:AppState;user:User;command:(type:string,payload:unknown)=>Promise<AppState & {commandResult?: {orderId: string}}>;workspaceRequest:<T=any>(path:string,body?:unknown,method?:string)=>Promise<T>;refresh:()=>Promise<void>;notify:(message:string)=>void;busy:boolean;adminTarget:EmployeeAccount|null;setAdminTarget:(target:EmployeeAccount|null)=>void}
