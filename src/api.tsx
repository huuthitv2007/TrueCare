...
export async function request<T=any>(path:string, body?:unknown, method?:string):Promise<T> {
 ...
 const response=await fetch(path,{method:method||(body===undefined?'GET':'POST'),credentials:'include',headers:body===undefined?undefined:{'Content-Type':'application/json',...(key?{'X-Idempotency-Key':key}:{})},body:body===undefined?undefined:JSON.stringify(body)});
