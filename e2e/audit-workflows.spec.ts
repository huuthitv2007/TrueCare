...
const emptyState = (name: string) => {
  ...
  state.attendance = [{ date: businessDate(), status: "worked", updatedAt: new Date().toISOString(), reason: "E2E setup" }];
  return state;
};

test("admin product delete retries without requesting an admin reason", async ({ page }) => {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/admin/products/bulk-actions") {
      ...
      state.products[0].archived = true;
      state.products[0].deletedAt = "2026-09-11T00:00:00Z";
...
test("employee cannot see delete actions and an expired session returns to login", async ({ page }) => {
  const state = emptyState("employee");
  let expired = false;
  ...
  await page.route("**/api/**", async (route) => {
    ...
    expired = true;
    return route.fulfill({ status: 401, json: { error: { code: "SESSION_EXPIRED", message: "Phiên hết hạn" } } });
  });
  await page.goto("/products");
